"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RefreshTokenUser = void 0;
var GenerateRefreshToken_1 = require("../provider/GenerateRefreshToken");
var typeorm_1 = require("typeorm");
var GenerateToken_1 = require("../provider/GenerateToken");
var RefreshToken_1 = require("../entity/RefreshToken");
var dayjs = require("dayjs");
var RefreshTokenUser = /** @class */ (function () {
    function RefreshTokenUser() {
    }
    RefreshTokenUser.prototype.execute = function (refresh_token) {
        return __awaiter(this, void 0, void 0, function () {
            var token, refreshToken, listRefresh, generateToken, refreshTokenExpired, generateRefreshToken, newRefreshToken, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        token = {};
                        refreshToken = new RefreshToken_1.RefreshToken();
                        this._repository = (0, typeorm_1.getRepository)("RefreshToken");
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 9, , 10]);
                        return [4 /*yield*/, this._repository.find({
                                where: {
                                    uid: refresh_token
                                }
                            })];
                    case 2:
                        listRefresh = _a.sent();
                        if (!(listRefresh.length >= 0)) return [3 /*break*/, 7];
                        generateToken = new GenerateToken_1.GenerateToken();
                        return [4 /*yield*/, generateToken.execute(listRefresh[0].user)];
                    case 3:
                        token = _a.sent();
                        refreshTokenExpired = dayjs().isAfter(dayjs.unix(listRefresh[0].expiresIn));
                        if (!refreshTokenExpired) return [3 /*break*/, 6];
                        // Delete user refreshtoken
                        return [4 /*yield*/, (0, typeorm_1.getConnection)()
                                .createQueryBuilder()
                                .delete()
                                .from(RefreshToken_1.RefreshToken)
                                .where("userId = :id", { id: listRefresh[0].userId })
                                .execute()];
                    case 4:
                        // Delete user refreshtoken
                        _a.sent();
                        generateRefreshToken = new GenerateRefreshToken_1.GenerateRefreshToken();
                        return [4 /*yield*/, generateRefreshToken.execute(listRefresh[0].user)];
                    case 5:
                        newRefreshToken = _a.sent();
                        return [2 /*return*/, { token: token, refreshToken: newRefreshToken }];
                    case 6: return [3 /*break*/, 8];
                    case 7: return [2 /*return*/, {
                            status: 404,
                            errors: [
                                'Refresh token inválido!'
                            ]
                        }];
                    case 8:
                        ;
                        return [3 /*break*/, 10];
                    case 9:
                        error_1 = _a.sent();
                        return [2 /*return*/, {
                                status: 400,
                                errors: [
                                    'Problemas na geração do token ' + error_1
                                ]
                            }];
                    case 10: return [2 /*return*/, { token: token, refreshToken: refreshToken }];
                }
            });
        });
    };
    return RefreshTokenUser;
}());
exports.RefreshTokenUser = RefreshTokenUser;
//# sourceMappingURL=RefreshTokenUser.js.map